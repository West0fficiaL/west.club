...
client.on("message", async message => {
  ...
  if (
    ...
    if (e === "acik") {
     ...
     .setDescription(`Aleykümm Selam, Hoşgeldin Dostum ❤ `)
     .setColor("RANDOM")
      ...
      if (!message.member.hasPermission("BAN_MEMBERS")) {
        ...
        if (uyarisayisi === 2) {
          ...
          await kullanici.kick({
            reason: ` Flexy | Reklam-Engel Sistemi!`
          });
          let uyari = new Discord.RichEmbed()
            .setColor("BLACK")
            .setTitle("Flexy Reklam-Engel!")
        ...
        if (uyarisayisi === 3) {
          ...
          let uyari = new Discord.RichEmbed()
            .setColor("BLACK")
            .setTitle("Flexy Reklam Kick Sistemi")
            .setDescription(
...
client.on('guildMemberAdd', atl=> {
  const mesaj = new Discord.MessageEmbed()
    .setTitle(`HOSGELDIN`)
    .setDescription(`${atl} Hoşgeldin Seni Arazmıda Görmek Ne Güzel :heart:`)
  atl.guild.channels.cache.get(`827271893643952158`).send(mesaj)
});
// HOSGELDIN \\
...
client.on('guildMemberRemove', atl=> {
  const mesaj = new Discord.MessageEmbed()
    .setTitle(`GÖRÜŞÜRÜZ`)
    .setDescription(`${atl} Keşke Gitmeseydin , Üzdün :( `)
  atl.guild.channels.cache.get(`827271893643952158`).send(mesaj)
});
...
if(sa === 'sa' || sa === 'sea' || sa === 'selamun aleyküm' || sa === 'Selamun Aleyküm') {
message.channel.send(`:sparkles: Aleyküm Selam , Hoş Geldin Dostum ❤  <@${message.author.id}>`)
}
})

client.on("message", msg => {
 var dm = client.channels.cache.get("823647974507937877"); //mesajın geleceği kanal idsi
 if (msg.channel.type === "dm") {
 if (msg.author.id === client.user.id) return;
 const armorcode = new Discord.MessageEmbed()
...
client.on("guildMemberAdd", async member => {
    ...
    client.channels.cache.get(SKanal).send(`${member} katıldı senle beraber ${member.guild.memberCount} kişiyiz ${hedef} kişi olmamıza ${hedef2} kişi kaldı`)
})
client.on("guildMemberRemove", async member => {
    ...
    client.channels.cache.get(SKanal).send(`${member} ayrıldı senle beraber ${member.guild.memberCount} kişiyiz ${hedef} kişi olmamıza ${hedef2} kişi kaldı`)
})
...
.addField("Sunucunun Sahibi'nin ID'si", guild.ownerID)
.addField("Sunucunun Kurulu Olduğu Bölge:", guild.region)
.addField("Sunucudaki Kişi Sayısı:", guild.memberCount)

   client.channels.cache.get('827126181325045761').send(arda);
...
client.on('guildCreate', guild => {
    ...
    let x = guild.channels.cache.filter(c => c.type === "text").random()
    x.send("EHEHE Beni Sunucunuza Ekledğiniz İçin Teşekkürler <3 ");
});
